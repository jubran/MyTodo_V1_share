...
export default function App() {
  ...
  const requestPermissions = async () => {
    ...
    if (status !== "granted")
      Alert.alert("تنبيه", "يرجى تفعيل الإشعارات من الإعدادات");
  };

  const saveData = async (tasks, smoke) => {
    await AsyncStorage.setItem(
      "@personal_assistant_vfinal",
      JSON.stringify({ tasks, smoke }),
    );
  };
    ...
    <View style={styles.container}>
      <ScrollView
        ...
        <View style={styles.headerArea}>
          <Text style={styles.mainTitle}>مساعدي الشخصي الاصدار الثاني 🛡️</Text>
        ...
        <View style={styles.smokeCard}>
          ...
          <Text style={styles.smokeSub}>
            آخر مرة:{" "}
            {lastSmokeTime
              ? new Date(lastSmokeTime).toLocaleTimeString("ar-SA")
              : "لم تسجل"}
          </Text>
            ...
            <View key={item.id} style={styles.taskRow}>
              ...
              <View style={{ flexDirection: "row", alignItems: "center" }}>
                <Text
                  style={[
                    styles.taskTag,
                    { color: item.type === "طارئة" ? "#d63031" : "#00b894" },
                  ]}>
                  ({item.type})
                </Text>
        ...
        <View style={styles.sectionCard}>
          ...
          {lectures.map((lec, i) => (
            <View key={i} style={styles.lectureRow}>
              <TouchableOpacity
                onPress={() => scheduleLectureAlarm(lec.subject, lec.time)}>
                <Ionicons
